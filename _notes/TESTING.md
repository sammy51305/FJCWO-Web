...
共 **479 個測試**，分布在 8 個 app。

> 最近一次全站測試：2026-09-07 執行 `python manage.py test`，**479/479 全數通過、無 regression**（System check 亦無問題）。

### `apps/accounts/tests.py`（124 個）
...
| `SensitiveProfileFieldsTest` | #13-1 必填欄位大改：七個必填各自空白都擋下、樂器／聲部／畢業年份選填可留空、Registration 存得下聲部、身分證字號格式驗證與轉大寫、遮蔽只留末四碼（未填時為空字串）、通訊錄與名冊報表不外洩身分證字號與住址、核准時四個敏感欄位整批帶進帳號、沒填樂器也能核准、既有帳號欄位空著仍可正常使用 |
...
| `FeeReportTest` | 團員自助申報（S2 現金 / S4 轉帳）：我的會費列期別/可申報、現金申報建 reported（記收款幹部、金額快照）、缺收款幹部被擋、轉帳申報記末五碼(無收款幹部)、末五碼非 5 位數字被擋、已申報/已繳不可重複、作廢後可重新申報、撤回自己的待確認、不可撤回已繳/他人 |
| `PaymentConfigTest` | 轉帳收款設定（S4）：存取控制（團員無權限/幹部可看）、儲存結構化欄位（代號/名稱/戶名/帳號）到單例設定、帳號存檔自動去除 dash 與空白、上傳 QR Code 圖檔保存 |
