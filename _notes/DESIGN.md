...
比照排練出席、財產借用、會費繳納、請假統計都已經有的列印功能，通訊錄目前沒有。

**延伸想法**：之後可能要做「演出海報設計」，海報上的名單要能直接從某場演出的參演名單抓取。
...
### 2. 演出／排練建立後，讓校友主動選擇是否參與

目前系統**沒有**「幹部建好演出/排練後，系統主動詢問團員是否參加」這個機制，
只有反向的「請假」（[LeaveRequest](DESIGN.md)：預設出席，例外才需要主動操作申請請假）。
如果要做「主動 RSVP 確認參加」，需要想清楚：
- 跟現有 `LeaveRequest`（請假）、`PerformanceAttendance`（演出出席確認）三者的關係是什麼？
...
`GuestMember` model 已經存在（`apps/events/models.py`），設計上就是「非正式團員、演出時
借調的外援」，資料上已經能出現在分譜分配（`PartAssignment.guest_member`）跟演出出席裡，
