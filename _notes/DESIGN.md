...
> 最後更新：2026-09-08（#13-1 決定二定案並實作敏感個資欄位加密，#13-1 至此全部完成；新增 #16 金鑰管理強化待辦；#13-5／#13-6 演出出席意願三態完成、演出請假廢除；測試站已上線 Render＋Neon 並加 robots.txt／noindex；#10 選定路線 A；#4 確認申報需身分證字號；#13-3 公告三欄已完成）
...
Email 仍是 console backend（第 2 點「Email 不能省」**尚未解決**，要測 #11 校友匯入的寄信流程必須填真 SMTP）。
...
| 風險 | 說明 | 對策 |
|------|------|------|
| **SMTP 沒設定好** | 核准 300 筆 = 寄 300 封信。現行寄信失敗的備援是「把帳密顯示在畫面上給幹部」（見 `send_temp_password_email` docstring），300 筆逐一抄寫不可行 | **匯入前先確認寄信通得了**（連動 #10：測試站原訂「不接真 Email」，此功能會推翻該簡化）|
