...
**為什麼會費與收支明細分開？**
會費有「期別 + 每人繳沒繳」的追蹤需求，和一般收支性質不同；兩者要如何串（確認繳費 → 自動入帳、
按收款日認列年度收支）是 §9 P3 的範圍，P1 尚未串接（此階段勾「已繳」不會產生 FinanceRecord）。

會費期別/繳納的 view 與 nav（移進「財務」下拉）、刪除層級（期別刪除限管理員、PROTECT 擋已用期別），
詳見附錄五 §9。
