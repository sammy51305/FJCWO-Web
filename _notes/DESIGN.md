...
> 最後更新：2026-08-09（附錄五 #10 部署討論擴充為候選路線 A 雲端 PaaS／B 家機自架＋NAS；#11 匯出 Google Sheet）
...
- **已排除**：Google Sheets 當資料庫（app 咬 SQL/關聯/Django auth，等於重寫，不可行，見 #11）；Google Drive 當媒體（API 配額＋權限衝突，改用 R2 或本機/NAS 磁碟）。
