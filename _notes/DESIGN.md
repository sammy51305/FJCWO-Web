...
> 最後更新：2026-08-12（附錄五導入 P0/P1/P2 優先權；新增 #11 校友名單批次匯入、#12 首頁待審核提醒不完整；#10 補上線前須處理事項）
...
| 優先權 | 項目 | 卡在哪 |
|:---:|------|--------|
| **P0** | #11 校友名單批次匯入（Google 表單 → 系統） | 趕上線用，方向已定案，可直接實作 |
| **P0** | #10 部署 Web 供幹部測試 | 待與幹部討論放哪、用什麼資料 |
| **P1** | #12 首頁待審核提醒不完整（幹部漏審風險） | 方向已明確，可直接實作 |
| **P1** | #8 補請假（過了時間還能不能請假） | 待與幹部討論規則 |
