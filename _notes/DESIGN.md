...
> 最後更新：2026-08-04（演出請假 §4.22，附錄五 §2 定案實作完成）
...
### 9. 會費系統重構：期別主檔 ＋ 團員自助申報 ＋ 自動入帳（規劃，方向已定案，待實作）

2026-08-04 與幹部討論定案的方向。以下為**實作藍圖**，尚未動工。動工時依「分階段範圍」逐階段做。
...
| 階段 | 內容 |
|------|------|
| P1 | `FeePeriod` 主檔 CRUD；`MembershipFee` 改 FK + status + amount 快照；會費繳納報表改讀期別；nav 把會費移進財務 |
| P2 | 團員自助申報（**現金**）＋ 幹部確認工作流；刪除層級（撤回/作廢/管理員硬刪）；首頁通知 |
| P3 | 確認繳費自動產生 FinanceRecord 入帳；`當年度收支` 列印報表（按收款日年份）|
| P4 | 轉帳掃碼繳費：管理員上傳固定 QRCode + 轉帳帳號文字；末五碼對帳；財務確認 |

Migration 順序（P1）：① 建 `FeePeriod`；② `MembershipFee.period` 轉 FK（現有自由文字資料需搬遷或清空——實作前確認 DB 現況）；③ 加 status / payment_method / account_last5 / finance_record 欄位。

> ✅ 方向全部定案（2026-08-04）：FeePeriod 固定金額、現金+轉帳兩種繳費、團員撤回/幹部作廢/管理員硬刪、
> 現金收付制按收款日認列、確認自動入帳、會費移進財務。**待實作**，依上表分階段進行。
