...
> 最後更新：2026-09-08（測試站接上 Gmail SMTP，#10 完成；#13-1 決定二定案並實作敏感個資欄位加密，#13-1 全部完成；新增 #16 金鑰管理強化待辦；#13-5／#13-6 演出出席意願三態完成、演出請假廢除；測試站已上線 Render＋Neon 並加 robots.txt／noindex；#10 選定路線 A；#4 確認申報需身分證字號；#13-3 公告三欄已完成）
...
- `Registration` 要不要加 `section` 欄位（決定表單收不收聲部）。
- 匯入的檔案格式：CSV 上傳，還是貼上試算表內容？Google 表單匯出的 CSV 首欄是「時間戳記」，需忽略。
- 匯入採預覽確認制（先顯示解析結果與錯誤列，幹部確認後才寫入）還是直接寫入。
- 重複匯入同一份檔案的冪等處理（依 CLAUDE.md 慣例優先考慮 `get_or_create`）。
