...
| # | 需求 | 優先權 | 狀態 |
|:--:|------|:---:|------|
| 1 | 團員資料必填欄位大改（見下） | **P0** | ✅ 已實作（2026-09-07），見 §4.2；儲存方式（決定二）仍待議 |
...
明文 ＋ 存取控管，或欄位層級加密，尚未取捨。列出當時的論點供續談：

| | 支持 | 反對 |
|---|------|------|
| 明文＋控管 | 與現有 phone/email 一致、不引入金鑰管理；加密欄位無法做 DB 層 unique 與搜尋 | DB 或備份檔外流即全部可讀 |
| 欄位加密 | DB 被拖走時仍是密文 | 金鑰要有地方放，而 #10 部署方向未定——若走路線 B 家機自架，金鑰與 DB 同機，防護力接近零 |

**這一項不擋 #11 的表單發出**——表單要收哪些欄位與資料庫怎麼存無關。
但要留意：**先做明文等於押注**，日後若改加密需要一次資料搬遷（讀出、加密、寫回），
不是改個欄位型別就好。

> **與 #10 部署位置連動（2026-09-07 補）**：上表「反對加密」那格的理由（金鑰與 DB 同機）
> 只在**家機自架**時成立。**正式站若放雲端**，DB 在他人機器上、金鑰放平台環境變數，
> 兩者分開，加密就有實質意義。**故決定二要與「正式站放哪」一起決定**，不能各決各的。
> 詳見 #10「正式上線但伺服器還沒架好、又要免費」。
