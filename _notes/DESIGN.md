...
| 聲部 | `User.section` → `SectionType` | ❌ | ⚠️ `Registration` 目前**沒有**此欄位，要收就得加 |

**不收**：密碼、身分證字號、戶籍地址（後者見下方「與 #4 的關係」）。
...
**2026-08-12 決議：先不收**，待確認政府申報實際需要哪些欄位後再議（見 #4）。
若日後決定要收，須一併設計「誰看得到、要不要加密或限制查詢」，不可只是加欄位。
...
2. **這批新欄位有三個是敏感個資**（身分證字號、住址、出生年月日）。#11 原本決議
   「先不收，待確認申報需求」，現在等於推翻該決議。要一併決定：誰看得到（團員通訊錄目前
   幹部可看電話/Email）、要不要加密儲存、報表與匯出要不要遮蔽。**不可以只是加欄位。**
