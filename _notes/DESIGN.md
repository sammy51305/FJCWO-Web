...
> 最後更新：2026-09-09（#13-2／#13-9／#13-10 改名「入團申請」、請假頁分部長提示、保險費分類，#13 全部結案；#13-4 樂譜庫存只列總譜；#13-7／#13-8 請假截止改當天 23:59、過期樣式明確；#11 批次匯入完成，P0 全數結案；測試站接上 Gmail SMTP，#10 完成；#13-1 決定二定案並實作敏感個資欄位加密，#13-1 全部完成；新增 #16 金鑰管理強化待辦；#13-5／#13-6 演出出席意願三態完成、演出請假廢除；測試站已上線 Render＋Neon 並加 robots.txt／noindex；#10 選定路線 A；#4 確認申報需身分證字號；#13-3 公告三欄已完成）
...
- [附錄四：待評估項目（未修正）](#附錄四待評估項目未修正)
- [附錄五：待開發功能構想（未設計，待討論）](#附錄五待開發功能構想未設計待討論)
...
**目前的優先順序**（P0/P1/P2 的定義見 [WORKFLOW.md](WORKFLOW.md) 標記慣例）：
...
| ✅ | #13-2 / #13-9 / #13-10（改名、請假提示、保險費分類）| **已完成**（2026-09-09），見下方各項 |
| **P2** | #14 分部長（聲部負責人） | 由 #13-9 衍生，實作方式待決定 |
| **P2** | #4 每年 12/25 前團員名單申報提醒 | 已確認申報需身分證字號（2026-08-29）；其餘欄位待確認 |
| **P2** | #16 金鑰管理強化（KEK／DEK 分層與輪替）| 加密已上線，此為強化；待決定 bootstrap 時機、並發防護、是否分層 |
...
### 4. 每年 12/25 前需提供團員名單向政府申報的提醒 — P2

全新構想。實作前建議先確認：
- ~~申報需要哪些欄位？~~ **已確認（2026-08-29）：申報需要身分證字號。**
  這解除了 #13-1 收敏感個資的疑慮——身分證字號有明確、具體的用途（一年一次的政府申報），
  不是「先收著以防萬一」。個資保護的配套（誰看得到、怎麼存）見 §#13-1 第 1 項的三個決定。
  其餘欄位（戶籍地址等）是否為申報必要仍待確認，但不擋 #11 表單發出。
...
### 8. 補請假（過了時間還能不能請假）— ✅ 已定案（2026-08-12）
...
> ✅ **已於 2026-09-09 實作完成**（見 §4.6）。以下為當初的定案內容。
>
> ✅ **已於 2026-08-12 demo 後定案**（見 #13-7）：採**方向 (b) 放寬截止點**，
> 明確定為「**排練當天 23:59**」；過了就無法申請，狀態直接列為**未出席（absent）**。
> **方向 (c) 團員事後補請、(d) 幹部代登記皆不做**——過了就是過了，不留補請假路徑。
>
> 實作注意：`Rehearsal` 只有單一 `date`、沒有結束時間，「當天 23:59」須由日期部分推算
> （該日 23:59:59），不是欄位裡的時間；現行 `rehearsal.date > now`（開始即鎖）要改掉。
> 配套 UI 見 #13-8：過期的請假按鈕要有明顯色差。

### 9. 會費系統重構：期別主檔 ＋ 團員自助申報 ＋ 自動入帳（S1–S4 已完成，2026-08-06）

2026-08-04 與幹部討論定案，2026-08-06 依分階段 S1–S4 全數實作完成。以下保留當初的實作藍圖與各階段設計，
...
| 階段 | 內容 | 狀態 |
|------|------|------|
| S1 | `FeePeriod` 主檔 CRUD；`MembershipFee` 改 FK + status + amount 快照；會費繳納報表改讀期別；nav 把會費移進財務 | ✅ 已完成（2026-08-04）|
| S2 | 團員自助申報（**現金**）＋ 幹部確認工作流；刪除層級（撤回/作廢/管理員硬刪）；首頁通知 | ✅ 已完成（2026-08-04）|
| S3 | 確認繳費自動產生 FinanceRecord 入帳；`當年度收支` 列印報表（按收款日年份）| ✅ 已完成（2026-08-06）|
| S4 | 轉帳掃碼繳費：管理員上傳固定 QRCode + 轉帳帳號文字；末五碼對帳；財務確認 | ✅ 已完成（2026-08-06）|

**S1 實際 migration（0003–0005，非破壞性保留既有資料）**：
...
（`account_last5` / `finance_record` 欄位延後到 S3/S4 階段再加。）

**S2 實作重點**：
- migration `0006` 加 `payment_method`（現金/轉帳，S2 只用現金）、`result_seen`（首頁通知，同 LeaveRequest）。
...
> 現金收付制按收款日認列、確認自動入帳、會費移進財務。**S1–S4 全數完成（2026-08-06），會費系統重構收尾。**

### 10. 部署 Web 供幹部測試 — P0 待討論

2026-08-09 記錄。目前系統只跑在本機開發環境，其他幹部無法實際連線試用。需要一個能讓幹部點來點去測試的部署，實作前待討論：
...
#### 定案：先走路線 A（2026-09-07）

需求是「今天就要能連上、而且免費」，路線 B 要重灌機器、當天做不完，故**先建 A 的測試站**，
B 仍是長期方向（測試站可長成正式站），兩者不衝突。**Render（網站）＋ Neon（Postgres）**，
不用 Render 自家免費 DB——它 30 天過期，資料整包不見。

**已補齊的部署缺件**（實際跑過 gunicorn 驗證：公開頁 200、登入 302、幹部頁 200、static 200）：

| 檔案 | 作用 |
|------|------|
| `render.yaml` | Blueprint：服務定義與環境變數骨架，機密值標 `sync: false` 不進 repo |
| `build.sh` | 建置指令：`pip install` → `collectstatic` → `migrate`，`set -o errexit` 讓任一步失敗即中止 |
| `.python-version` | 釘 3.13.1（Django 6 需要 ≥3.12）|
| `requirements.txt` | 新增 `gunicorn`／`whitenoise`／`dj-database-url` |

`config/settings.py` 的三處改動與**為什麼這樣寫**：

- **`DATABASE_URL` 優先、否則沿用 `DB_*`**：託管資料庫給的是一條連線字串，本機給的是五個變數。
  用 if/else 分流而非二選一改寫，本機開發與測試完全不受影響。
- **WhiteNoise 用 `CompressedStaticFilesStorage`，刻意不用帶 manifest 的版本**：
  manifest 會在檔名加雜湊，少一個檔案就整頁 500，對測試站而言風險大於快取效益。
- **安全設定（強制 https、secure cookie）以環境變數開關、預設關閉**：
  不能寫成 `if not DEBUG`——**Django 跑測試時會把 `DEBUG` 設為 False**，
  那樣 `SECURE_SSL_REDIRECT` 會讓整套測試被 301 打掛。

**demo 密碼改讀環境變數**：`seed_demo.py` 的 `DEMO_PASSWORD` 改為
`os.environ.get('DEMO_PASSWORD', 'demo1234')`。預設值只給本機，測試站必須用環境變數蓋掉——
這解決了本節「上線前必須處理的兩件事」的第 1 點（DEMO.md 裡那組密碼已進 repo、等於公開）。

**免費方案的三個限制**（測試可接受，正式上線前要解）：休眠（15 分鐘無人使用即睡，喚醒約 1 分鐘）、
**磁碟為暫時性**（重新部署會清空上傳的樂譜 PDF／收據，QR 可重新產生）、
Email 原為 console backend，**已於 2026-09-08 接上 Gmail SMTP**（應用程式密碼），
第 2 點「Email 不能省」就此解決，設定步驟見 [SETUP.md](SETUP.md) 情境 E-8。
⚠️ 一般 Gmail 帳號有每日寄送額度，#11 一次核准三百位校友時可能撞到限流，屆時要分批。

建置步驟（Neon 註冊 → Render Blueprint → 灌 fixtures 與 demo 資料）見
[SETUP.md](SETUP.md) 情境 E。

#### 正式上線但伺服器還沒架好、又要免費（2026-09-07 討論）

前提是「正式站（真團員資料）」而非測試站，且家機（路線 B）還沒就緒。
結論先講：**免費撐得住正式站，但要先補掉那個會掉檔案的洞。**

**測試站的三個限制，放到正式站的嚴重度完全不同**：

| 限制 | 測試站 | 正式站 |
|------|------|------|
| 15 分鐘休眠、喚醒約 1 分鐘 | 可接受 | 難看但不致命（團員一天開不了幾次）|
| **磁碟為暫時性** | 可接受 | ❌ **致命**——重新部署即失去團員上傳的樂譜 PDF、收據 |
| Email 為 console backend | 可接受 | ❌ 必須解——收不到臨時密碼信等於拿不到帳號 |

所以問題不是「免費能不能當正式站」，而是**「怎麼免費地補掉掉檔案那個洞」**。

**三個可行組合**

| 組合 | 內容 | 取捨 |
|------|------|------|
| 一：最小改動 | Render free ＋ Neon ＋ **Cloudflare R2**（媒體改存物件儲存）| 補掉磁碟洞，只剩休眠。要加 `django-storages`、改 storage 設定，約半天；**注意 §4.19 分譜下載的權限判斷不能因此變成公開連結** |
| 二：最像正式站 | **Oracle Cloud Always Free** VM（永久免費、不休眠、有持久磁碟）＋ Cloudflare Tunnel ＋ Let's Encrypt | 架構與路線 B 幾乎相同，**今天在 Oracle 上做的之後搬回家機幾乎原封不動**。代價：要綁卡驗證、ARM 機器常搶不到、維運（更新／備份／憑證）全自理 |
| 三：混搭 ← **建議** | 組合一 ＋ 真 SMTP（Gmail 應用程式密碼或 Brevo 免費額度）| 零維運、每個環節都是託管服務。就是把組合一的 Email 洞一併補上 |

> 各家免費額度會變動，實際數字以註冊當下官網公告為準，本文件不記可能過期的數字。

**不論選哪個組合，正式上線這五件事沒得談**：

1. **備份**——免費方案的備份保留期通常很短。自己排定期 `pg_dump` 倒出來另存。**這件事沒做，其他都是空的。**
2. **真的 SMTP**（Gmail 應用程式密碼即足夠）。
3. **清掉 demo 帳號**（跑 `clear_demo`，並確認 `DEMO_PASSWORD` 不是預設值）。
4. **`DEBUG=False`、`SECRET_KEY` 換新**（`render.yaml` 已處理）。
5. **維持可攜性**——本系統是 Django ＋ Postgres ＋ S3 相容儲存，三者皆為標準介面，
   免費方案縮水或收攤時，搬家是換連線字串的等級而非重寫。**這是免費方案敢當正式站的前提。**

**與 #13-1 決定二的連動（重要）**

#13-1 決定二（敏感個資明文 vs 加密）當初建議明文，理由之一是「若走家機自架，
金鑰與 DB 同機，加密防護力接近零」。**正式站若放雲端，這個理由不成立**——
DB 在他人機器上，而金鑰放平台環境變數、資料放託管 DB，兩者是分開的，加密此時有實質意義。

> **所以決定二的答案取決於正式站放哪**：家機 → 明文可接受；雲端 → 傾向加密。
> 兩件事要一起決定，不能各決各的。

**付費逃生口**（記錄供比較）：Render 最低付費方案約每月 US$7（約 NT$220），
即無休眠、有持久磁碟，上述麻煩一次消失。樂團有會費收入，這金額屬預算零頭，
值得讓幹部自行選擇「省這筆」或「省麻煩」。

**上線前必須處理的兩件事（2026-08-12 補）**：

1. **清掉 demo 帳號、換掉 demo 密碼**。本機 demo 資料的 `demo_*` 帳號密碼統一是 `demo1234`，
   而這組密碼寫在 [DEMO.md](DEMO.md) 裡、已進 repo。本機無妨，但若把那套資料原封搬上測試站，
   等於公開 repo 裡躺著一組真實可用的憑證。部署時要嘛不帶 demo 資料，要嘛先改密碼並刪掉這些帳號。
2. **Email 不能省**。原本設想「測試站不接真 LINE/Email」，但 #11 的校友匯入靠「核准 → 寄臨時密碼信」
   完成最後一哩；開發環境的 console backend 只把信印在終端機，測試站若沿用，
   校友根本收不到密碼、整條流程斷在這裡。要嘛接真 SMTP，要嘛先確認匯入 demo 只在本機做。

### 11. 校友名單批次匯入（Google 表單 → 系統）— ✅ 已完成（2026-09-08）

2026-08-12 記錄。**背景**：系統趕不上原定上線時間，因此先開 Google 表單請校友填寫基本資料，
待系統上線後再把這批回覆匯入。需要一個「批次匯入」入口。

#### 關鍵前提：這不是新流程，是既有流程的批次入口

系統已有完整的「校友報到 → 建帳號」鏈路（見 §4.2），匯入應該**接上去**而不是另起爐灶：
...
Registration（status=pending）
        ↓ ★ 本次要做的：批次核准
_create_member_with_temp_password()  ← 既有，不動
        ↓
建 User + 寄臨時密碼信 + must_change_password=True
        ↓
校友首次登入 → ForcePasswordChangeMiddleware 強制設定新密碼  ← 既有，不動
```

**定案（2026-08-12）**：

1. **匯入目標＝`Registration`（不直接建 `User`）**。理由：DESIGN §4.2 已明訂「`Registration` 是這個帳號
   怎麼來的唯一紀錄」（所以已核准的申請不可刪除）。直接建 `User` 會讓這批帳號沒有來源紀錄，
   破壞既有稽核原則。代價只是多一道核准步驟 → 用**批次核准**（勾選多筆一起核准）解決。
2. **權限：僅幹部＋管理員**，團員無權限。比照既有慣例 `@login_required` + `if not request.user.is_officer`。
3. **密碼沿用既有臨時密碼信**，不另做設定密碼連結（零開發，趕上線優先）。

#### 表單要請校友填的欄位

> 以下為 **#13-1 定案後（2026-08-29）** 的版本：必填改成姓名／Email／出生年月日／住址／
> 手機／LINE ID／身分證字號，樂器、聲部、畢業年份降為選填。原 2026-08-12 版的
> 「不收身分證字號、戶籍地址」已被推翻——#4 已確認申報需要身分證字號（見下方「與 #4 的關係」）。

| 表單題目 | 對應欄位 | 必填 | 備註 |
|---------|---------|:---:|------|
| 真實姓名 | `Registration.name` | ✅ | |
| Email | `Registration.email` | ✅ | 同時是產生 username 的來源（取 `@` 前段）；`User.email` 為 `unique` |
| 出生年月日 | `Registration` 新增欄位 | ✅ | 敏感個資，遮蔽規則見 #13-1 決定一 |
| 住址 | `Registration` 新增欄位 | ✅ | 敏感個資，同上 |
| 手機 | `Registration.phone` | ✅ | 原為選填，改必填 |
| LINE ID | `Registration` 新增欄位 | ✅ | **使用者自己填的 LINE 帳號**，與 `User.line_user_id`（LINE Bot 的 internal id）是兩回事，不可覆用 |
| 身分證字號 | `Registration` 新增欄位 | ✅ | 敏感個資；用途是 #4 政府申報。表單需做合理性提示，匯入時檢查格式 |
| 樂器 | `Registration.instrument` → `InstrumentFamily` | ❌ | 表單填的粗分類（豎笛／薩克斯風／打擊樂器）對應**族群**這層，九個實際填答值全部對得上（見 §4.2）|
| 輔大入學年／就讀科系 | `Registration.alumni_info` | ❌ | 原文照存，不自動拆成 `grad_year` |
| 聲部 | `Registration` 新增欄位 → `SectionType` | ❌ | #13-1 連帶定案：`Registration` **要加 `section` 欄位**，表單收聲部 |
| 畢業年份 | `Registration.grad_year` | ❌ | 表單要明講**西元**（欄位是 `PositiveSmallIntegerField`）|

**不收**：密碼（理由見下節）。

> **樂器／聲部／畢業年份降為選填的連帶影響**：依樂器分組的頁面要處理 null——
> 通訊錄（§4.2）現有「未分類」分支可沿用，**演出分譜下載（§4.19）需補上同樣的守衛**。

#### 為什麼不在 Google 表單收密碼

1. 表單回覆會落在 Google Sheet，等於**一整欄明文密碼**攤在試算表裡，任何拿到連結的人都看得到，
   且留存於 Google 的版本歷史，事後刪除也未必清得掉。
2. 校友多半重用其他網站的密碼 → 這張表外洩會波及他們的其他服務。
3. 表單無法做密碼強度驗證，也無法雜湊。

系統既有機制已經解決這件事：臨時密碼由系統隨機產生（`get_random_string(10)`）、只存在於那封信裡，
且 `must_change_password=True` 保證校友首次登入一定會被強制換掉。

#### 實作前必須先確認的風險

| 風險 | 說明 | 對策 |
|------|------|------|
| ~~SMTP 沒設定好~~ | 核准 300 筆 = 寄 300 封信。現行寄信失敗的備援是「把帳密顯示在畫面上給幹部」（見 `send_temp_password_email` docstring），300 筆逐一抄寫不可行 | ✅ **已於 2026-09-08 接上 Gmail SMTP**（見 SETUP 情境 E-8）；匯入前仍應用 `sendtestemail` 再確認一次 |
| **一次寄太多信被限流** | 批次核准可能觸發 SMTP 服務的寄送速率限制 | 實作時決定要不要分批／限制單次核准筆數 |
| **樂器對不上 FK** | 表單若開放自由填寫，會出現「長笛 / Flute / 長笛(短笛)」等寫法 | 表單用下拉；匯入時對不上的列出來給幹部人工處理，不靜默略過 |
| **Email 重複** | 同一人填兩次、或校友之間共用 email（`User.email` 是 `unique`）| 匯入時偵測並報告，不直接建立 |
| **民國/西元混填** | `grad_year` 是數字欄位，收到 `114` 跟 `2025` 都存得進去 | 表單明講西元；匯入時做合理範圍檢查 |

#### 實際回覆的盤點（2026-09-07，15 筆）

表單已於 2026-08-12 起開始收件。實際回覆對照定案欄位，發現五件事：

| 發現 | 影響 | 處置 |
|------|------|------|
| **15 筆中 13 筆沒有 Email** | Email 是 username 來源與臨時密碼收件處，**沒有就建不了帳號** | Email 題是 8/19 前後才加進表單的，先前的人沒被問到。幹部依 LINE ID 逐一補齊後再匯入 |
| 有一筆證號填「N/A」（港澳團員）| 格式驗證會擋下 | 放寬為身分證＋居留證兩種格式；兩者皆無者幹部端留空（見 §4.2）|
| 「輔大入學年／就讀科系」對不上 `grad_year` | 民國入學年 ＋ 科系混在一格，格式六七種 | 新增 `alumni_info` 原文照存，`grad_year` 留空（見 §4.2）|
| 樂器只有四個名稱與 `InstrumentType` 完全相同 | 匯入會大量對不上 | `Registration.instrument` 改指 `InstrumentFamily`，九個值全部對得上（見 §4.2）|
| **「報名第二聲部」不是 `SectionType`** | 表單那題填的是**第二志願樂器**（薩克斯風／豎笛／鋼琴），與系統的聲部（第一部、第二部…）語意不同 | ⬜ **待處理**：匯入時不可對到 `section`。要嘛新增「第二志願樂器」欄位，要嘛忽略該題由幹部人工處理 |

表單另有三題（自備樂器、排練時間確認、願不願意當工作人員）是**這場音樂會專屬的報名資訊**，
不屬於團員檔案，匯入時忽略、留在試算表供幹部參考。

#### 實作（2026-09-08 完成）

**檔案**：`apps/accounts/imports.py`（解析與對應）、`views.py` 的 `registration_import`／
`_import_registration_rows`／`_bulk_approve_registrations`／`_approve_registration`。

原本列為待決定的四件事，實作時的取捨：

| 待決事項 | 決定 | 理由 |
|---------|------|------|
| `Registration` 要不要加 `section` | 已加（#13-1 時一併做）| — |
| 檔案格式 | **CSV 上傳** | 貼上試算表內容要處理跳脫與換行，收益不對等 |
| 預覽確認制 vs 直接寫入 | **一步匯入 ＋ 詳細報告** | 見下 |
| 冪等 | **以 Email 為鍵，已存在就跳過並報告** | 幹部照報告修好 CSV 後可整份重傳 |

**為什麼不做預覽確認制**：預覽必須在兩個 request 之間保存解析結果，而那裡面有身分證字號——
存進 session 就是**明文落在 `django_session` 表**，直接抵銷掉剛做的欄位加密（#13-1 決定二）。
「錯誤列不寫入 ＋ 以 Email 冪等」能達到同樣效果，且沒有這個副作用。

**為什麼用標題列比對欄位、不看順序**：表單題目會增修——實際回覆裡 **Email 那題就是
2026-08-19 前後才加的**，之前的欄位位置全部往後推一格。認標題文字（含關鍵字比對，
因為題目很長且帶說明）才不會因為改題目而整批錯位。

**解析的三個實務細節**：
- **UTF-8 with BOM**：Google 匯出的 CSV 帶 BOM，要用 `utf-8-sig` 解碼，否則第一個標題會多出隱形字元而對不到。
- **日期格式不統一**：實際資料有 `1999/12/30` 也有 `1999-12-30`，逐一嘗試多種格式。
- **樂器對照只轉必要的幾個**：`打擊樂器→打擊樂`、`長笛/短笛→長笛`、`鋼琴→其他`、`無→空`。
  **對不上的不猜**，留空並在報告中列出讓幹部指定（樂器本來就是選填）。

**略過的列一律列出原因**（DESIGN 風險表要求「不靜默略過」）：缺 Email、缺姓名、
同檔案內 Email 重複、Email 已有系統帳號、Email 已有申請紀錄。
其中**缺 Email 是實際資料裡最常見的**（15 筆回覆有 13 筆缺），訊息直接寫「請向本人補齊後重新匯入」。

**批次核准**：審核頁每列加勾選框，一次核准多筆。三個設計點：
- **單筆與批次共用 `_approve_registration()`**，避免兩套規則各自演化。
- **一筆失敗不影響其他筆**：300 筆裡有幾筆 Email 撞號是常態，整批 rollback 只會讓幹部無從下手。
  失敗的逐筆列出原因，成功的照常建帳號。
- **寄信失敗不讓核准失敗**：帳號已經建好了，退回顯示帳密讓幹部自行轉達（沿用單筆的既有行為）。
- 表格每列本來就各有自己的核准／拒絕 form，**巢狀 form 是無效 HTML**，
  所以批次的 form 放在表格外，勾選框與送出鈕用 HTML5 的 `form=""` 屬性指回去。

#### 與 #4（政府申報）的關係（#11）

原本考慮「請校友填一次表單」成本高，是否順便收申報用的身分證字號、戶籍地址等敏感個資。
2026-08-12 曾決議「先不收，待確認申報需求」，**2026-08-29 推翻**：#4 已確認
**申報需要身分證字號**，用途明確，故隨這次表單一併收齊，不再發第二次表單。

配套的個資保護不是「日後再說」，已隨 #13-1 定案（見該節「敏感個資的三個決定」）：
可見範圍採**幹部可見、預設遮蔽**，遮蔽做成 model property；儲存方式（明文或加密）
仍待討論，但不擋表單發出——表單收什麼與資料庫怎麼存是兩件事。

### 12. 首頁待審核提醒不完整（幹部視角）— P1

2026-08-12 demo 準備時發現。**期望是「所有待審核事項都能在首頁看到」，現況只做到一半。**

**現況**（`apps/public/views.py` 的 `index`）：幹部區塊只查兩種待審，另兩種完全沒有提醒。

| 待審核類型 | Model / 狀態 | 首頁有提醒？ | 審核頁 |
|-----------|-------------|:---:|--------|
| 入團申請 | `Registration` `pending` | ✅ | `/accounts/register/review/` |
...
「沒有待審核的請假申請」——他很容易讀成「沒有請假要我審」，但實際上可能有好幾筆躺在審核頁等他。
**這會讓幹部漏審，且漏了不會有任何跡象。**
...
> 定位為 P1：功能本身都在、審核頁從選單進得去，不擋上線；但缺了會讓幹部漏審，
> 屬於「上線後要補」。若 demo 後幹部反應強烈，可調升為 P0。

### 13. Demo 後幹部回饋的改動清單（2026-08-12 demo）— 逐項標優先權

demo 給幹部看之後收到的 10 項改動需求，**2026-08-12 當晚已逐項回覆確認、方向全部定案**。

| # | 需求 | 優先權 | 狀態 |
|:--:|------|:---:|------|
| 1 | 團員資料必填欄位大改（見下） | **P0** | ✅ 已完成（2026-09-08），三個決定皆定案並實作，見 §4.2 |
| 2 | 「校友報到申請」改名為「入團申請」 | P2 | ✅ 已完成（2026-09-09）|
...
| 8 | 請假按鈕過期後要明確標色（如變暗）| P1 | ✅ 已完成（2026-09-09），見 §4.6 |
| 9 | 請假頁提示「記得通知自己的分部長」 | P2 | ✅ 已完成（2026-09-09）靜態文字；分部長另立 **#14** |
| 10 | 財務收支新增「保險費」分類 | P2 | ✅ 已完成（2026-09-09）|

#### 第 1 項：團員資料必填欄位（P0，且卡住 #11 的表單）

**幹部要求（2026-08-12 定案）**：必填改為
**姓名、Email、出生年月日、住址、手機、LINE ID、身分證字號**；
樂器、聲部、畢業年份改為選填。

> Email 必填一事在回饋當下漏列，隨即補確認。**帳號建立機制因此維持不動**
> （`_create_member_with_temp_password()` 仍以 email 的 `@` 前段產生 username、
> 臨時密碼寄至該 email）。

**為什麼是 P0**：#11 的 Google 表單還沒發出去，**表單要收哪些欄位完全取決於這個決定**。
表單一旦發出去再改，等於要請所有校友重填。這件事排在發表單之前。

**實作前必須先解決的問題**：

1. **Email 重複要有處理方式** —— `User.email` 是 `unique`，同一人填兩次、
   或校友之間共用信箱都會讓匯入失敗（#11 風險表已列）。表單要提醒「請填本人專用信箱」，
   匯入時偵測重複並列給幹部處理，不可靜默略過。
2. **這批新欄位有三個是敏感個資**（身分證字號、住址、出生年月日）→ **見下方三個決定**。
3. **LINE ID 與現有 `line_user_id` 不是同一個東西** —— 現有欄位存的是 LINE Bot 取得的
   internal user id，不是使用者自己填的 LINE 帳號。要新增獨立欄位，不要覆用。
4. 樂器/聲部改選填後，**依樂器分組的頁面要處理 null**（通訊錄 §4.2、演出分譜 §4.19
   都以樂器分組；通訊錄現有「未分類」分支可參考）。

##### 敏感個資的三個決定（2026-08-29）

決策當下查核到的現況，作為判斷基礎：**通訊錄的電話／Email 已經是幹部限定**
（`member_directory.html` 那兩欄被 `{% if user.is_officer %}` 包住，一般團員看不到），
通訊錄報表整頁亦為幹部限定，且**全站目前沒有任何 CSV／Excel 匯出**（匯出是 #15 的未來構想）。
換言之現成的曝光點只有兩個頁面，都已擋在 `is_officer` 之後。

**決定一：可見範圍 —— 幹部可見、預設遮蔽（採用）**

列表與報表**不顯示完整值**（顯示末四碼或不顯示）；要看完整值須進該團員詳情頁、
點「顯示完整資料」。本人永遠看得到自己的。

- 選它的理由：身分證字號一年只用一次（#4 申報），日常沒有幹部需要看；
  但鎖到「只有管理員」又會讓申報變成一個人的事，卡在單點。遮蔽是成本最低的中間解。
- 曾考慮並放棄：**只有管理員**（每次申報都得管理員本人來，且權限判斷要從 `is_officer`
  改成 `is_admin_role`，多養一套分支）、**幹部全可見**（零成本，但把申報用的識別碼
  跟日常聯絡資訊放同一層，沒有理由）。
- **實作硬性要求：遮蔽做成 model 的 property**（如 `masked_national_id`），
  不可以靠各個 template 自己記得遮——否則日後多一個頁面就漏一個，這是資安邊界不是顯示細節。

**決定二：儲存方式 —— 欄位層級加密（採用，2026-09-08 定案）**

| | 支持 | 反對 |
|---|------|------|
| 明文＋控管 | 與現有 phone/email 一致、不引入金鑰管理 | DB 或備份檔外流即全部可讀 |
| **欄位加密（採用）**| DB 被拖走時仍是密文 | 需管金鑰；加密欄位無法做 DB 層 unique 與搜尋 |

**選它的理由**：正式站傾向放雲端，DB 在他人機器上、金鑰放平台環境變數，兩者分離時
加密才有實質意義（若走家機自架、金鑰與 DB 同機，防護力接近零，見 #10）。
另一個現實理由是**時機**：當時只有 demo 資料，改的成本接近零；等 #11 把三百位校友匯進去
再改，就是一次要停機處理的資料搬遷。

> 若日後正式站改放家機，這個決定仍然無害——只是多付一點成本，不需要改回明文。

**代價（已知並接受）**：加密欄位無法做 DB 層 unique、無法用 SQL 搜尋或排序。
這三個欄位目前都只用於顯示；日後若要「找出這個月生日的人」，得在 Python 端算
（團員數量級是數百，可接受）。

**與決定一的關係**：兩者是互補的兩層，不重疊——**加密防的是 DB／備份外流，
遮蔽防的是頁面曝光**。少了任何一層另一層都補不上。

實作見 §4.2「敏感個資的欄位加密」。

**決定三：既有帳號補齊 —— model 可空 ＋ 表單必填 ＋ 提示補填（採用）**

- **model 層一律 `blank=True`／`null=True`**：既有團員這些欄位都是空的，
  設 `null=False` 會讓 `migrate` 直接卡在既有資料上。
- **必填由表單層把關**：新申請、幹部代建帳號一律要填。
- **既有帳號不強制擋**：登入後在個人資料頁提示補填即可，**不做 middleware 強制導向**。
  理由是這批欄位對「看排練時間、請假、繳會費」毫無必要，用 middleware 擋人會在上線初期
  把團員擋在門外；真要申報時由幹部從名單看誰沒填、直接催那幾個人。

**連帶決定**：`Registration` **要加 `section`（聲部）欄位**，Google 表單收聲部。

> **狀態**：決定一、三已定案，決定二待討論。#11 的表單欄位至此全部確定，**表單可以發出去**。

#### 第 5、6 項：演出意願確認 ＋ 演出併入排練（定案 2026-08-12）

原本擔心這兩項會重演 #2 否決 RSVP 的理由（主動 RSVP 與請假打架），
**但加上「待確認」第三態之後就自洽了**——兩者不是競爭關係，而是同一筆出席紀錄的兩種輸入：

| 團員做了什麼 | 狀態 |
|-------------|------|
| 按「確認參加」 | 確認參加 |
| 對演出那場請假 | 不參加 |
| **兩者都沒做** | **待確認** ← 由指定聯絡人負責聯繫 |

**第 6 項定案**：廢掉獨立的演出請假，**演出視為該活動的最後一場排練**
（10 場排練 → 演出自動列為第 11 項），名稱顯示為「演出活動」，當成一次特別團練處理，不另外開流程。
原 §4.22 的「演出請假審核」頁**改為承載第 5 項的演出意願統計**。

**要廢掉的既有實作**：`PerformanceLeaveRequest` model、其審核頁與首頁通知、
`PerformanceAttendance.on_leave`（2026-08-04 完成，見 §4.22）。
**既有資料**：正式環境尚未上線，該 model 只有 demo 資料，直接砍掉即可（見 §Demo 資料清理）。

**實作建議（尚未定案的細節）**：

- **三態用單一欄位承載**，不要用兩個 boolean。現有 `PerformanceAttendance` 是
  `confirmed`（是否到場）＋ `on_leave`（請假）兩個 boolean，會產生 4 種組合，
  其中「confirmed=True 且 on_leave=True」是矛盾狀態、沒有任何程式碼阻止。
  建議改成 `intent` 三選一（`pending` 預設／`confirmed`／`declined`）。
- **「事前意願」與「事後到場」要分開**。現有 `confirmed` 是「當天有沒有到」的語意，
  與「事前說要不要來」不同。若共用一個欄位，會失去「說要來但沒到」這個最需要追蹤的情況。
- **統計頁按聲部分組**顯示三態人數，方便分部長認領自己聲部的待確認名單（連動 #14）。
- **指定聯絡人怎麼指定**：建議等 #14 分部長做好後由分部長負責；
  在那之前先由幹部從統計頁自行聯繫，不另做欄位。

#### 第 7、8 項：#8 補請假就此收斂（定案 2026-08-12）

**決議**：請假截止點改為**排練當天 23:59**，過了就**無法申請、狀態直接列為未出席（absent）**，
**不提供任何補請假路徑**（#8 的方向 c「團員事後補請」與 d「幹部代登記」皆不做）。

這同時回答了 #8 懸而未決的兩半，該項可標記為已決議 —— 詳見 §8。

**實作注意**：`Rehearsal` 只有單一 `date` 欄位、沒有結束時間，
所以「當天 23:59」要由 `date` 的**日期部分**推算（該日 23:59:59），不是欄位裡的時間。
現行判斷是 `rehearsal.date > now`（一到開始時刻就鎖），要改成比對當日結束。

第 8 項是配套 UI：現況過期的請假連結雖已是 disabled，但視覺上看不出來，需要加明顯色差
（如變暗＋游標禁止），讓團員一眼知道還能不能按。

#### 第 3 項：公告分上中下三欄 ＋ 一個實際 bug（定案 2026-08-12）

**版面**：由上而下 **幹部限定 → 團員限定 → 公開**，權限比照現有 `visibility` 機制
（幹部看三欄、團員看下兩欄、未登入只看公開）。

**同時要修一個 demo 當天發現的 bug**：一般民眾看不到公開公告。
**根因不是權限邏輯**——`_visible_announcements()` 對未登入者正確回傳 public 公告，
未登入直接開 `/announcements/` 也確實是 200、公開公告都顯示得出來（已驗證）。
問題在 `templates/base.html`：**公告連結被包在 `{% if user.is_authenticated %}` 裡**，
...
#### 第 2 項：「校友報到申請」改名為「入團申請」

**確認**：純顯示文字，不動 model 類別名（`Registration`）、URL name 與 template 檔名——
那是重構，不是這項需求，改了只增加風險。

> ✅ **已於 2026-09-09 實作完成**。換掉的是使用者與幹部看得到的字：
> template 標題／麵包屑／導覽列／首頁按鈕、`Registration.Meta.verbose_name`（會出現在 Django Admin）、
> `seed_demo` / `clear_demo` 印給幹部看的標籤。
>
> 連帶把程式碼註解與 docstring 的用語也一起換掉——留一半舊名會讓後人以為系統裡有兩種東西，
> 正是這次改名要消除的混淆。**`migrations/0002_registration.py` 裡的舊 verbose_name 不動**：
> 遷移檔是歷史紀錄，改它等於竄改已套用過的歷史。新的 `0010_alter_registration_options`
> 才是描述現況的那一筆。

#### 第 4 項：樂譜庫存清單只列總譜（語意已澄清）

**確認**：樂譜庫存的清單**只呈現總譜，分譜完全不出現在清單上**；要看分譜一律從總譜詳情頁進去。
...
**確認**：請假頁加一行靜態提示「記得通知自己的分部長」，**先不做系統自動通知**。

分部長本身是新的人員關係（系統目前完全沒有這個概念，已確認 codebase 無 `section_leader` 之類的欄位），
**另立 #14 待辦**，之後再做。等 #14 完成後，這行提示可以升級成指名道姓、
甚至自動 LINE 通知，也能供 #5 的「待確認名單」分派使用。

> ✅ **已於 2026-09-09 實作完成**（見 §4.6）。提示出現在**兩個時機**，不是一個：
> 填表時的 alert，以及送出成功後的 message。只放表單上的話，那則提示會隨著送出跳頁一起消失，
> 而「該去通知分部長」正是送出**之後**才要做的事。
>
> 文字刻意寫成不指名的「自己的分部長」——系統還不知道誰是分部長（#14）。
> 同時說明「系統只把申請送到幹部審核，不會通知分部長」，讓團員知道這不是多此一舉的客套話。

#### 第 10 項：財務收支新增「保險費」分類

**確認**：`FinanceRecord.Category` 加一個 choice，排在樂譜費之後、會費之前（支出類群在一起）。

> ✅ **已於 2026-09-09 實作完成**。`0010_alter_financerecord_category` 只改 choices，
> 不動資料。新增／編輯表單與年度收支報表都是讀 `Category.choices` 動態產生，
> 沒有任何地方硬寫分類清單，所以加一行 model 就全站都有了——這是當初不把分類寫死在
> template 換來的好處。

#### Demo 資料清理

#13-6 要廢掉的 `PerformanceLeaveRequest` 只有 demo 資料（正式環境未上線），可直接砍。
demo 資料的建立與清理腳本、以及資料內容清單，見 [DEMO.md](DEMO.md)。

### 14. 分部長（聲部負責人）— P2

2026-08-12 由 #13-9 衍生。**系統目前沒有「誰是這個聲部的分部長」這個資訊**
（`User` 有 `section` 聲部欄位，但沒有任何地方記錄負責人）。

**已知的三個用途**（都先用替代方案擋著，做了才能升級）：

| 用途 | 目前的替代作法 | 有了分部長之後 |
|------|--------------|--------------|
| #13-9 請假提示 | 靜態文字「記得通知自己的分部長」 | 指名顯示、甚至自動通知 |
| #13-5 演出待確認名單 | 幹部從統計頁自行聯繫 | 按聲部分派給各分部長追 |
| 日常分部練習聯絡 | 無 | — |

**實作前要決定**：

- 分部長怎麼表示？候選：`SectionType` 上加負責人 FK（一個聲部一位）、
  `User` 上加 `is_section_leader` 旗標、或新增 `role=section_leader`。
  ⚠️ **不建議動 `role`**——現有 `is_officer` 依賴 role 判斷權限，
...
### 16. 金鑰管理強化：KEK／DEK 分層與輪替 — P2

2026-09-08 討論，**未定案**。#13-1 決定二的加密已上線（見 §4.2），這一項是它的強化，
不是它的前提——現況在「金鑰是隨機字串」的前提下已經擋得住主要威脅（DB／備份外流）。

#### 現況與已知弱點

| 現況 | 弱點 |
|------|------|
| `SHA-256(passphrase)` 直接推導出加密金鑰 | 推導太快、無 salt；passphrase 若是人想得出來的字串，暴力破解成本低 |
| 該金鑰同時扮演 Master／KEK／DEK（三層壓成一層）| 換金鑰＝所有密文解不開，只能全部重新加密 |
| 每加解密一個值都重算一次金鑰 | SHA-256 快所以無感，但換成慢 KDF 就會是效能問題 |
| 沒有輪替指令 | 「換金鑰」目前是「改環境變數然後祈禱」，是最大的操作地雷 |
| 沒有檢查 passphrase 強度 | 有人填 `fjcwo2026` 系統照收 |

#### 討論結論（已有共識的部分）

- **熵遠比迭代數重要**：迭代數從 10 萬拉到 60 萬只買到 6 倍；passphrase 從人腦產出換成
  隨機字串買到約 10²⁹ 倍。**真正的防線是啟動時檢查金鑰強度**（長度下限、拒絕開發預設值），
  那是確定性的保護；迭代數只是「萬一有人填了爛密碼」的薄保險。
- **迭代數 10 萬即可，不要 60 萬**：多 0.36 秒冷啟動換趨近於零的邊際安全。
- **輸入若保證高熵，正確的原語其實是 HKDF**（快且無安全損失）；PBKDF2 是為了拉伸低熵秘密而存在。
  兩條路都可行，取決於要不要信任「金鑰一定是隨機產生的」這個前提。
- **PBKDF2 不會拖慢測試**：只要金鑰列由 migration 建立、推導結果在 process 層快取，
  整個測試跑下來只執行一次（約 60ms）。
- **KEK／DEK 分層對本專案的價值是運維彈性，不是防護力**：分層與單層對「DB dump 外流」
  這個實際威脅的防護完全相同（兩者都是拿到 DB 但沒有環境變數就解不開）。
  分層買到的是「換 KEK 不用重新加密資料」——但本團只有數百筆，全部重新加密約兩秒。

#### 待決定

1. **Bootstrap 時機**：lazy 自動產生 / 明確指令 / **放進 `build.sh` 冪等執行（傾向）**。
   lazy 的危險是連到空的或錯的資料庫時會安靜地產生第二把 DEK。
2. **並發**：gunicorn 多 worker 同時啟動可能各自產生一把 DEK。
   需固定 pk 的單列表 ＋ `get_or_create`，讓資料庫當仲裁者。
   **產生兩把 DEK 之後，先寫入的資料就再也解不開了**，這個坑一定要先堵。
3. **要不要真的分層**（KEK wrap DEK，wrapped DEK 存 DB 單列表，salt 與迭代數跟著記錄走）。

#### 擬議的資料結構（若採用分層）

```
EncryptionKey（單列，pk 固定）
  wrapped_dek      text    ← KEK 包住的 DEK
  kdf_salt         bytes   ← 不是機密
  kdf_iterations   int     ← 記錄當時的參數，日後調高不會讓舊資料解不開
  algo_version     int     ← 演算法敏捷性（將來換 Argon2 用得到）
  created_at / rotated_at
```

**wrapped DEK 與資料放同一個 DB 不是問題**——KEK 不在那裡，拿到 dump 的人兩樣都拿到，
但沒有環境變數仍然無用。反之若把 wrapped DEK 也放環境變數，兩個秘密同籃，分層等於白做。

#### 沒有技術解的一件事

**KEK passphrase 遺失＝資料永久不可讀**，沒有後門、沒有救援；備份還原回來也沒用。
那串環境變數**必須有離線備份**（密碼管理器或紙本鎖起來），這是整個系統裡唯一一件
不能只存在雲端的東西。

> 對照工業界做法：SSD 韌體用 eFuse 當信任根，金鑰從不離開晶片。PaaS 上沒有等價物——
> KEK 最終是 process 讀得到的環境變數，信任鏈的底部是「誰控制 Render 帳號」而非硬體。
> 要真正的硬體信任根得用 KMS/HSM，但那要錢又綁雲，與「免費架站」的前提衝突。
