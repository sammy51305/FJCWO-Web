...
> 最後更新：2026-09-07（#13-1 必填欄位大改已實作，儲存方式仍待議；測試站已上線 Render＋Neon 並加 robots.txt／noindex；#10 選定路線 A；#4 確認申報需身分證字號；#13-3 公告三欄已完成）
...
**身分證字號**：
- 驗證只檢查格式（1 個英文字母 + 9 位數字），**不驗檢查碼**——居留證號等變體規則不同，
  驗太嚴會擋掉真實存在的號碼；目的是擋打錯，不是做身分驗證。
...
| 樂器 | `Registration.instrument` → `InstrumentType` | ❌ | **必須下拉選單**，選項需與系統 `InstrumentType` 完全一致 |
