...
from apps.accounts.models import SectionType, User
...
class PerformanceIntentTestCase(TestCase):
    """演出出席意願三態（#13-6：廢除演出請假，演出視為最後一場排練）"""

    def setUp(self):
        self.section = SectionType.objects.create(name='第一部')
        self.member = User.objects.create_user(
            username='pi_member', email='pi_member@test.local',
            password='testpass123', name='意願測試團員', role=User.Role.MEMBER,
            section=self.section,
        )
        self.other = User.objects.create_user(
            username='pi_other', email='pi_other@test.local',
            password='testpass123', name='沒表態的人', role=User.Role.MEMBER,
            section=self.section,
        )
        self.officer = User.objects.create_user(
            username='pi_officer', email='pi_officer@test.local',
            password='testpass123', name='意願測試幹部', role=User.Role.OFFICER,
        )
        self.venue = Venue.objects.create(name='意願測試場地', type='performance')
        self.event = PerformanceEvent.objects.create(
            name='意願測試音樂會', type=PerformanceEvent.Type.CONCERT,
            performance_date=timezone.now() + timedelta(days=30),
            performance_venue=self.venue,
        )
        self.set_url = reverse('events:performance_intent_set', args=[self.event.pk])
        self.report_url = reverse('events:performance_intent_report', args=[self.event.pk])

    def _intent_of(self, user):
        return PerformanceAttendance.objects.get(event=self.event, member=user).intent

    # ── 表態 ────────────────────────────────────────────────

    def test_unauthenticated_redirects_to_login(self):
        """未登入無法表態"""
        r = self.client.post(self.set_url, {'intent': 'confirmed'})
        self.assertRedirects(r, f'/accounts/login/?next={self.set_url}', fetch_redirect_response=False)

    def test_confirm_attendance(self):
        """按確認參加 → intent=confirmed，並記下表態時間"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'confirmed'})
        attendance = PerformanceAttendance.objects.get(event=self.event, member=self.member)
        self.assertEqual(attendance.intent, PerformanceAttendance.Intent.CONFIRMED)
        self.assertIsNotNone(attendance.intent_at)

    def test_decline_requires_reason(self):
        """不參加一定要填原因，否則不記錄"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'declined', 'decline_reason': ''})
        self.assertFalse(
            PerformanceAttendance.objects.filter(event=self.event, member=self.member).exists()
        )

    def test_decline_with_reason(self):
        """不參加＋原因 → intent=declined 並存下原因"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'declined', 'decline_reason': '當天有課'})
        attendance = PerformanceAttendance.objects.get(event=self.event, member=self.member)
        self.assertEqual(attendance.intent, PerformanceAttendance.Intent.DECLINED)
        self.assertEqual(attendance.decline_reason, '當天有課')

    def test_switching_back_to_confirmed_clears_reason(self):
        """改回確認參加要清掉原因，否則統計頁會顯示上一次的說法"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'declined', 'decline_reason': '當天有課'})
        self.client.post(self.set_url, {'intent': 'confirmed'})
        attendance = PerformanceAttendance.objects.get(event=self.event, member=self.member)
        self.assertEqual(attendance.intent, PerformanceAttendance.Intent.CONFIRMED)
        self.assertEqual(attendance.decline_reason, '')

    def test_intent_can_be_changed_before_performance(self):
        """演出前可以反覆改，不會產生第二筆紀錄（unique event+member）"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'confirmed'})
        self.client.post(self.set_url, {'intent': 'declined', 'decline_reason': '臨時有事'})
        self.assertEqual(
            PerformanceAttendance.objects.filter(event=self.event, member=self.member).count(), 1
        )
    ...
    def test_invalid_intent_value_ignored(self):
        """送出不在三態內的值不建立紀錄"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'whatever'})
        self.assertFalse(
            PerformanceAttendance.objects.filter(event=self.event, member=self.member).exists()
        )

    def test_get_does_not_change_intent(self):
        """GET 不改狀態（避免被預抓連結誤觸）"""
        self.client.force_login(self.member)
        self.client.get(self.set_url, {'intent': 'confirmed'})
        self.assertFalse(
            PerformanceAttendance.objects.filter(event=self.event, member=self.member).exists()
        )
    ...
    def test_intent_and_attended_are_independent(self):
        """「說要來」與「當天有沒有到」互不影響——說要來但沒到是最需要追蹤的情況"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'confirmed'})
        attendance = PerformanceAttendance.objects.get(event=self.event, member=self.member)
        self.assertEqual(attendance.intent, PerformanceAttendance.Intent.CONFIRMED)
        self.assertFalse(attendance.attended)   # 事後到場預設 False，不因表態而變
    ...
    def test_report_requires_officer(self):
        """統計頁為幹部限定"""
        self.client.force_login(self.member)
        r = self.client.get(self.report_url, follow=True)
        self.assertContains(r, '權限不足')

    def test_report_counts_three_states(self):
        """三態各自計數；沒表態的人算「待確認」"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'confirmed'})
        self.client.force_login(self.officer)
        r = self.client.get(self.report_url)
        self.assertEqual(r.context['totals']['confirmed'], 1)
        # officer 與 other 都沒表態
        self.assertEqual(r.context['totals']['pending'], 2)
        self.assertEqual(r.context['totals']['declined'], 0)

    def test_report_includes_members_without_any_record(self):
        """名單要從團員表反推——只掃 attendance 會把沒表態的人整批漏掉"""
        self.client.force_login(self.officer)
        r = self.client.get(self.report_url)
        self.assertContains(r, '沒表態的人')

    def test_report_groups_by_section(self):
        """依聲部分組，方便分部長認領自己聲部的待確認名單"""
        self.client.force_login(self.officer)
        r = self.client.get(self.report_url)
        self.assertContains(r, '第一部')

    def test_report_shows_decline_reason(self):
        """不參加的原因要顯示給幹部看，才知道要不要補人"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'declined', 'decline_reason': '當天有課'})
        self.client.force_login(self.officer)
        self.assertContains(self.client.get(self.report_url), '當天有課')

    def test_report_excludes_guests(self):
        """客座團員不列入意願統計（他們不是正式團員）"""
        User.objects.create_user(
            username='pi_guest', email='pi_guest@test.local', password='x',
            name='客座樂手', role=User.Role.GUEST,
        )
        self.client.force_login(self.officer)
        self.assertNotContains(self.client.get(self.report_url), '客座樂手')

    # ── 演出詳情頁 ──────────────────────────────────────────

    def test_event_detail_shows_intent_buttons(self):
        """演出詳情頁有表態入口"""
        self.client.force_login(self.member)
        r = self.client.get(reverse('events:event_detail', args=[self.event.pk]))
        self.assertContains(r, '確認參加')
        self.assertContains(r, self.set_url)

    def test_event_detail_shows_current_intent(self):
        """表態後詳情頁顯示目前狀態"""
        self.client.force_login(self.member)
        self.client.post(self.set_url, {'intent': 'confirmed'})
        r = self.client.get(reverse('events:event_detail', args=[self.event.pk]))
        self.assertEqual(r.context['my_intent'], PerformanceAttendance.Intent.CONFIRMED)

    def test_event_detail_locks_after_performance(self):
        """演出開始後詳情頁不再顯示表態按鈕"""
        past_event = PerformanceEvent.objects.create(
